...
var createFilter = function createFilter2(include2, exclude, options) {
  ...
  const includeMatchers = ensureArray(include2).map(getMatcher);
...
// node_modules/vite-plugin-glsl/src/loadShader.js
...
function checkIncludedDependencies(path, root) {
  ...
  dependencies == null ? void 0 : dependencies.forEach(
    (dependency) => recursiveDependency = recursiveDependency || checkIncludedDependencies(dependency, root)
  );
...
function loadChunks(source, path, extension, warn, root) {
  const unixPath = path.split(import_path3.sep).join(import_path3.posix.sep);
  if (checkRecursiveImports(unixPath, warn)) {
    return recursiveChunk;
  }
  source = removeSourceComments(source);
  let directory = (0, import_path3.dirname)(unixPath);
  ...
  if (include.test(source)) {
    ...
    source = source.replace(include, (_, chunkPath) => {
      ...
      if (!chunkPath.indexOf("/")) {
        const base = (0, import_process.cwd)().split(import_path3.sep).join(import_path3.posix.sep);
        chunkPath = base + root + chunkPath;
      }
      const directoryIndex = chunkPath.lastIndexOf("/");
      directory = currentDirectory;
      if (directoryIndex !== -1) {
        directory = (0, import_path3.resolve)(directory, chunkPath.slice(0, directoryIndex + 1));
        chunkPath = chunkPath.slice(directoryIndex + 1, chunkPath.length);
      }
      let shader = (0, import_path3.resolve)(directory, chunkPath);
      if (!(0, import_path3.extname)(shader))
        shader = `${shader}.${extension}`;
      const shaderPath = shader.split(import_path3.sep).join(import_path3.posix.sep);
      (_a = dependentChunks.get(unixPath)) == null ? void 0 : _a.push(shaderPath);
      return loadChunks(
        (0, import_fs2.readFileSync)(shader, "utf8"),
        shader,
        extension,
        warn,
        root
...
function loadShader_default(source, shader, options) {
  ...
  return {
    dependentChunks,
    outputShader: compress ? typeof compress !== "function" ? comressShader(output) : compress(output) : output
  };
}

// node_modules/vite-plugin-glsl/src/index.js
...
function src_default({
  ...
  const filter = createFilter(include2, exclude);
  const prod = false;
  return {
    enforce: "pre",
    ...
    async transform(source, shader) {
      ...
      const module = moduleGraph == null ? void 0 : moduleGraph.getModuleById(shader);
...
vite-plugin-glsl/src/index.js:
   ...
   *)
*/
//# sourceMappingURL=vite-plugin-glsl.js.map
